...
irqreturn_t so2_kbd_interrupt_handle(int irq_no, void *dev_id)
{
	...
	printk(LOG_LEVEL "IRQ %d: scancode=0x%x (%u) pressed=%d ch=%c\n",
		irq_no, scancode, scancode, is_key_press(scancode), get_ascii(scancode));
		...
		size_t size, loff_t *offset)
{
	...
	memmove(tmp, data->buf, data->size); 
...
static int so2_kbd_init(void)
{
	...
	printk(LOG_LEVEL "Request keybd INT\n");
	/* TODO 2: request IRQ */
	if (request_irq(I8042_KBD_IRQ,
		so2_kbd_interrupt_handle,
                IRQF_SHARED, MODULE_NAME, &devs[0]) != 0) {
		err = -EBUSY;
		printk(LOG_LEVEL "Request keybd failed INT\n");
